        ...
        'images', coalesce((select jsonb_agg(jsonb_build_object(
          ...
          select image_item, section_position, image_position from (
            select image_item, section_position, image_position,
              row_number() over (partition by image_item->>'image_url' order by section_position, image_position) duplicate_rank
            from (
              ...
              where pd.portal_id=p.id and section.item->>'type' in ('gallery','image_comparison')
                and coalesce((section.item->>'visible')::boolean,true)
            ) raw_images where image_item->>'image_url' is not null
              and image_item->>'image_url' <> '' and coalesce((image_item->>'visible')::boolean,true)
          ) unique_images where duplicate_rank=1 order by section_position, image_position limit 5
