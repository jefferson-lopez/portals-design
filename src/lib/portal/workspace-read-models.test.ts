...
test("migration ordering creates Stripe projection columns before read-model functions reference them", async () => {
  const readModels = await Bun.file(
    new URL("../../../supabase/migrations/20260817140000_workspace_read_models.sql", import.meta.url),
  ).text();
  const projection = await Bun.file(
    new URL("../../../supabase/migrations/20260817150000_stripe_connect_safe_projection.sql", import.meta.url),
  ).text();

  expect(readModels.indexOf("add column if not exists account_email text")).toBeGreaterThanOrEqual(0);
  expect(readModels.indexOf("add column if not exists account_email text")).toBeLessThan(
    readModels.indexOf("create or replace function public.get_home_workspace_summary()"),
  );
  expect(projection).not.toContain("alter table public.creator_stripe_accounts");
});
...
test("extended Stripe callers use the distinct projection RPC while onboarding keeps the legacy RPC", async () => {
  ...
  const onboarding = await Bun.file(
    new URL("../../app/api/billing/connect/onboarding/route.ts", import.meta.url),
  ).text();

  expect(complete).toContain('rpc("upsert_creator_stripe_account_projection"');
  expect(status).toContain('rpc("upsert_creator_stripe_account_projection"');
  expect(onboarding).toContain('rpc("upsert_creator_stripe_account"');
});
