-- Password protection is available for every portal plan.
...
-- Keep the trigger as a defense-in-depth path, but do not couple privacy to billing.
...
create trigger portals_enforce_premium_visibility
before insert or update of visibility on public.portals
for each row execute function public.enforce_portal_premium_visibility();
