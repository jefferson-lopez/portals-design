-- Paid portals expose only their sales projection to anonymous callers.
-- The published snapshot is fetched separately by the trusted server after
-- the authenticated buyer grant has been checked.
...
returns jsonb language sql stable security definer set search_path = public as $$
  select case when p.id is null then null else jsonb_build_object(
    'portal', jsonb_build_object(
      ...
      'cover_url', p.cover_url, 'allow_downloads', p.allow_downloads,
