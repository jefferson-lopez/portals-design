...
create or replace function public.delete_portal_block(
  target_portal_id uuid,
...
begin
  if not public.can_edit_portal(target_portal_id) then
    raise exception 'Not allowed to delete portal block';
  end if;

  delete from public.portal_blocks
  where id = target_block_id
    and portal_id = target_portal_id;
end;
$$;

grant execute on function public.update_portal_settings(uuid, text, text, text, text, text, public.portal_visibility, text, text, text, text, boolean, boolean, boolean, boolean, public.portal_theme, text, text, text, text) to authenticated;
grant execute on function public.delete_portal_block(uuid, uuid) to authenticated;
