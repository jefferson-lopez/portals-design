-- Expired upload reservations must not consume quota after an abandoned upload.
-- The API cleans them opportunistically, but quota calculations must also be
-- correct when storage cleanup was interrupted or never retried.
...
begin
  ...
  if premium then
    ...
    where portal_id=target_portal_id
      and (state='ready' or (state='reserved' and reservation_expires_at>now()));
  else
    select coalesce(sum(a.size_bytes),0) into used_bytes
    from public.portal_assets a
    join public.portals p on p.id=a.portal_id
    where p.owner_id=target_owner
      and not public.portal_has_premium(p.id)
      and (a.state='ready' or (a.state='reserved' and a.reservation_expires_at>now()));
  end if;
...
returns public.portal_assets language plpgsql security definer set search_path = public as $$
declare saved public.portal_assets; target_owner uuid; premium boolean; used_bytes bigint; quota_bytes bigint; max_file_bytes bigint;
begin
  ...
  quota_bytes := case when premium then 2147483648 else 104857600 end;
  max_file_bytes := 52428800;
  if actual_size_bytes <= 0 or actual_size_bytes > max_file_bytes then raise exception 'Uploaded asset exceeds plan file-size limit'; end if;
  ...
  if premium then
    select coalesce(sum(size_bytes),0)-saved.size_bytes into used_bytes
    from public.portal_assets
    where portal_id=saved.portal_id
      and (state='ready' or (state='reserved' and reservation_expires_at>now()));
  else
    select coalesce(sum(a.size_bytes),0)-saved.size_bytes into used_bytes
    from public.portal_assets a
    join public.portals p on p.id=a.portal_id
    where p.owner_id=target_owner
      and not public.portal_has_premium(p.id)
