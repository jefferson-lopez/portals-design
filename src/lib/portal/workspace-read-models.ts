...
export type ConnectStatusSummary = {
  ...
  verificationState: "active" | "needs_information" | "not_started" | "processing";
  lastSyncedAt: string | null;
  needsSync: boolean;
};

export function normalizeConnectStatusSummary(value: Json): ConnectStatusSummary {
  const record = value && typeof value === "object" && !Array.isArray(value)
    ? (value as Record<string, Json>)
    : {};
  const verificationState = record.verificationState;
  const normalizedVerificationState =
    verificationState === "active" || verificationState === "needs_information" || verificationState === "processing"
      ? verificationState
      : "not_started";
  ...
  return {
    ...
    requirementsPending: typeof record.requirementsPending === "number" ? record.requirementsPending : 0,
